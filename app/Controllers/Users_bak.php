...
{
   // REMOVE the constructor and the protected $authorization property
   ...
   public function edit($userId)
   {
      ...
      /*
      // Get all UNIQUE groups that exist in the system
      ...
      $data['allGroups'] = [];
      foreach ($groupsQuery->getResultArray() as $row) {
         $data['allGroups'][] = $row['group'];
      }*/

      // If no groups exist yet, provide some defaults
      if (empty($data['allGroups'])) {
         $data['allGroups'] = ['superadmin', 'admin', 'user'];
      }
      ...
      $data['allPermissions'] = []; // Skip permissions for now
      $data['user'] = $user;
   ...
   {
      // Get available groups for the admin to assign
      $db = db_connect();
      $groupsQuery = $db->table('auth_groups_users')
         ->select('group')
         ->distinct()
   ...
   public function store()
   {
      ...
      $selectedGroups = $this->request->getPost('groups') ?? ['user'];
